...
void sstp_send(gnutls_session_t* tls, void* data, uint16_t len)
{
  ssize_t sbytes = 0;
  int i;
...
void send_sstp_packet(gnutls_session_t* tls, int type, void* data, uint16_t len)
{
  sstp_header_t pkt_hdr;
  uint16_t sstp_hdr_len = sizeof(sstp_header_t);
  uint16_t total_length = sstp_hdr_len + len;
  void *pkt = NULL;
...
void send_sstp_data_packet(gnutls_session_t* tls, void* data, uint16_t len) 
{
...
void send_sstp_control_packet(gnutls_session_t* tls, uint8_t msg_type,
			      sstp_attribute_header_t* attrs, uint16_t len)
{
  sstp_control_header_t ctrl_hdr;
  uint16_t ctrl_hdr_len = sizeof(sstp_control_header_t);
  uint16_t ctrl_len = ctrl_hdr_len + len;
  void* data;
...
void init_sstp(gnutls_session_t* tls)
{
  ...
  attr_hdr.packet_length = htons(len);
  
  memcpy(data, &attr_hdr, len);
  memcpy(data + sizeof(sstp_attribute_header_t), &attr_data, sizeof(uint16_t));
